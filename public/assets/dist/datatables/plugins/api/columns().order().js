 ...
 * `dt-api column().orderBy()` method, but there is no plural equivalent. While
 * multi-column ordering can be set using `dt-api orderBy()` that method requires
 * that column indexes be used.
 *
 * This plug-in provides the plural `columns().orderBy()` method so you can set
 ...
 *    table.columns( [ 1, 2 ] ).orderBy( 'desc' ).draw();
 *
 *  @example
 *    // Multi-column sorting with individual direction for the columns
 *    table.columns( [ 1, 2 ] ).orderBy( [ 'desc', 'asc' ] ).draw();
 *
 *  @example
 *    // Multi-column sorting based on a name selector
 *    table.columns( [ 'sign_up_date:name', 'user_name:name' ] ).orderBy( 'desc' ).draw();
 */

$.fn.dataTable.Api.register( 'columns().orderBy()', function ( dir ) {
  return this.iterator( 'columns', function ( settings, columns ) {
    ...
    new $.fn.dataTable.Api( settings ).orderBy( a );
  } );
} );
